...
import { toBase64 } from '#app/utils/misc'
...
// Remove or modify functions that depend on OptionalTeam
// For example, you might want to use a string union type instead:
type Team = 'RED' | 'BLUE' | 'YELLOW' | 'UNKNOWN'

const kodyProfiles: Record<Team, { src: string; alt: string }> = {
	RED: {
...
// Update other image sets to use the new Team type
const kodySnowboardingImages: Record<Team, ImageBuilder> = {
...
// Define the missing image sets
const kodySkiingImages: Record<Team, ImageBuilder> = {
...
const kodyOnewheelingImages: Record<Team, ImageBuilder> = {
...
const kodyPlayingSoccerImages: Record<Team, ImageBuilder> = {
...
const kodyBackFlippingImages: Record<Team, ImageBuilder> = {
...
export function getRandomSportyKody(team?: Team) {
	const activities = [
	...
	if (team) {
		...
		const teams: Team[] = ['RED', 'BLUE', 'YELLOW', 'UNKNOWN']
		const randomTeam = teams[Math.floor(Math.random() * teams.length)] ?? 'UNKNOWN'
		return set[randomTeam]
	}
}

// Update getRandomFlyingKody similarly
